...
from disco.data.constants import ELEMS, STD_RESIDUES
...
class TokenArray:
    ...
    def get_annotation(self, category):
        """Gets annotation values for all tokens.

        Args:
            category: Name of the annotation to retrieve.

        Returns:
            List of annotation values, one per token.
        """
        return [token._annot[category] for token in self.tokens]

    def set_annotation(self, category, values):
        """Sets annotation values for all tokens.

        Args:
            category: Name of the annotation to set.
            values: Iterable of values; must have the same length as the
                number of tokens.

        Raises:
            AssertionError: If ``len(values)`` does not match the token count.
        """
    ...
    def get_values(self):
        """Gets token values for all tokens.

        Returns:
            List of token values (typically integer residue or element IDs).
        """
