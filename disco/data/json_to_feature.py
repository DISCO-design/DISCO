...
from disco.data.json_parser import add_entity_atom_array, remove_leaving_atoms
...
class SampleDictToFeatures:
    """Converts a sample dictionary into feature dicts and atom arrays.

    Builds a full AtomArray from the input JSON sample, adds inter-entity
    bonds, computes annotations, and produces featurized tensors suitable
    for model input.

    Args:
        single_sample_dict: Input sample dictionary describing sequences and
            entities.
        task_manager: TaskManager instance used for masking during training.
        backbone_only: If True, use backbone-only mode for representative
            atom masks.
    """

    def __init__(self, single_sample_dict, task_manager, backbone_only):
    ...
    def get_atom_array(
        self, atom_array: AtomArray | None = None, bb_only: bool = False
    ) -> AtomArray:
        """
        ...
        atom_array = self.add_atom_array_attributes(
            atom_array, self.entity_poly_type, bb_only
        )
        return atom_array
    ...
    def get_feature_dict(
        self,
        atom_array=None,
        called_during_generation: bool = False,
        bb_only: bool = False,
        ...
        if not called_during_generation:
            masking_result = self.task_manager.mask(atom_array)

            atom_array, token_array = (
                masking_result.atom_array,
                masking_result.token_array,
            )
