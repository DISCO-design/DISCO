...
from disco.data import ccd
from disco.data.ccd import get_ccd_ref_info
...
from disco.data.utils import get_starts_by
...
class AddAtomArrayAnnot:
    ...
    def add_token_mol_type(
        ...
        for start, stop in zip(starts[:-1], starts[1:], strict=False):
    ...
    @staticmethod
    def add_distogram_rep_atom_mask(
        atom_array: AtomArray, bb_only: bool = False
    ) -> AtomArray:
        """
    ...
    def find_equiv_mol_and_assign_ids(
        atom_array: AtomArray, entity_poly_type: dict[str, str] | None = None
    ) -> AtomArray:
        """
        ...
        for start, stop in zip(mol_starts[:-1], mol_starts[1:], strict=False):
    ...
    def add_ref_feat_info(
        atom_array: AtomArray, atom_idxs: np.ndarray | None = None
        ...
        Args:
            ...
            atom_idxs (Optional[np.ndarray]): Indices of atoms in the atom array to update
                                              reference information for. If this is not None
                                              then we'll only change ref information for these
                                              atom indices. If it is None, then we update ref information
                                              for all atoms.
        ...
        if atom_idxs is None:
            atom_idxs = np.arange(len(atom_array))

        info_dict, atom_array_to_update = {}, atom_array[atom_idxs]
        for ccd_id in np.unique(atom_array_to_update.res_name):
            # create ref conformer for each CCD ID
            ref_result = get_ccd_ref_info(ccd_id)
            if ref_result:
                ref_space_uids = atom_array_to_update[
                    atom_array_to_update.res_name == ccd_id
                ].ref_space_uid

                for space_uid in np.unique(ref_space_uids):
                    if ref_result:
                        info_dict[space_uid] = [
                            ref_result["atom_map"],
                            ref_result["coord"],
        ...
        ref_pos = []  # [N_atom, 3]
        ref_charge = []  # [N_atom]
        for atom in atom_array_to_update:
        ...
        new_ref_pos = np.array(ref_pos)
        new_ref_charge = np.array(ref_charge).astype(int)
        new_ref_mask = np.array(ref_mask).astype(int)

        if hasattr(atom_array, "ref_pos"):
            ref_pos = atom_array.ref_pos
            ref_pos[atom_idxs] = new_ref_pos

            ref_charge = atom_array.ref_charge
            ref_charge[atom_idxs] = new_ref_charge

            ref_mask = atom_array.ref_mask
            ref_mask[atom_idxs] = new_ref_mask

        else:
            ref_pos = new_ref_pos
            ref_charge = new_ref_charge
