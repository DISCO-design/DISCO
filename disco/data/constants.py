...
# Prefix used in JSON input files to indicate a ligand path (e.g. "FILE_path/to/mol.sdf")
LIGAND_FILE_PREFIX = "FILE_"
...
STD_RESIDUES = (
    PRO_STD_RESIDUES | RNA_STD_RESIDUES | DNA_STD_RESIDUES | MASK_STD_RESIDUES
)

PRO_STD_RESIDUES_VALS_SET = set((PRO_STD_RESIDUES | MASK_STD_RESIDUES).values())

PRO_RES_IDX_TO_RESNAME = {
    val: key for key, val in (PRO_STD_RESIDUES | MASK_STD_RESIDUES).items()
}
...
# The DNA_RES_IDX_TO_RESNAME dict is used for outputing DNA sequences to result files. As such,
# because the DNA_STD_RESIDUES dict sets keys to be f"D{DNA_RES}" we remove the "D" prefix
# in the logging dictionary.
PROT_STD_RESIDUES_THREE_TO_ONE = {
    v: k for k, v in PROT_STD_RESIDUES_ONE_TO_THREE.items()
}
DNA_RES_IDX_TO_RESNAME = {v: k[1] for k, v in DNA_STD_RESIDUES.items()}
RNA_RES_IDX_TO_RESNAME = {v: k for k, v in RNA_STD_RESIDUES.items()}
PRO_RES_IDX_TO_RESNAME_ONE = {
    val: PROT_STD_RESIDUES_THREE_TO_ONE[key]
    for key, val in (PRO_STD_RESIDUES | MASK_STD_RESIDUES).items()
}
...
LMPNN_ELEMENT_DICT = dict(
    zip(LMPNN_ELEMENT_LIST, range(1, len(LMPNN_ELEMENT_LIST)), strict=False)
)
...
OUR_ELE_TO_LMPNN_MAP.update(
    {"OG": "UUO", "MC": "UUP", "TS": "UUS", "NH": "UUT", "MO": "MB"}
)
